...
export const CheckoutForm: React.FC = () => {
  ...
  const handleRetry = async () => {
    try {
      const result = await apiSimulation.retrySubmit(formData)
      if (result.success) {
        console.log('Retry successful:', result.data?.orderId)
      }
    } catch (error) {
      ...
      >
        {({ isSubmitting, errors, touched, values, handleBlur }) => (
          <Form className="space-y-8">
            ...
            {/* Retry Button */}
            {apiSimulation.error && apiSimulation.canRetry && (
              <div className="retry-container">
                <Button
                  type="button"
                  variant="secondary"
                  onClick={handleRetry}
                  loading={apiSimulation.isRetrying}
                ...
                >
                  ...
                  Retry ({apiSimulation.retryCount}/3)
                </Button>
              </div>
            )}
